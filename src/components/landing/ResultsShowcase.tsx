          ...
          {results.map((r, i) => {
            ...
            return (
              <motion.div
                ...
                className="relative bg-card/60 backdrop-blur-sm rounded-2xl p-8 border border-border/50 hover:border-primary/40 transition-all duration-500 overflow-hidden group cursor-default"
              >
                {/* Top glow line */}
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-4">
                    <motion.div
                      ...
                      className={`w-12 h-12 rounded-2xl ${isPrimary ? 'bg-primary/10' : 'bg-gold/10'} flex items-center justify-center`}
                    >
                      <r.icon className={`w-6 h-6 ${isPrimary ? 'text-primary' : 'text-gold'}`} />
                    </motion.div>
                  ...
                  <motion.p
                    className={`font-display text-4xl font-black mb-2 ${isPrimary ? 'text-primary' : 'text-gold'}`}
                    initial={{ scale: 0.5 }}
                    whileInView={{ scale: 1 }}
                    viewport={{ once: true }}
                ...
                <motion.div
                  className={`absolute -bottom-4 -right-4 w-20 h-20 rounded-full ${isPrimary ? 'bg-primary/5' : 'bg-gold/5'}`}
